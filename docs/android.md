...
Android-версия — это Tauri 2 APK с тем же Monaco-редактором, Rust-компилятором и встроенным Rust-интерпретатором Funo. Обычные Funo-программы действительно выполняются внутри приложения: JDK, сеть и внешний процесс для запуска не нужны. Интерфейс адаптирован для телефона: нижняя навигация, выдвижной проводник, крупные touch-элементы, сворачиваемая панель результатов и safe-area отступы.
...
- настоящий запуск обычных Funo-программ встроенным process-free интерпретатором без JDK;
- функции и рекурсия, переменные, условия, циклы, массивы, коллекции, f-строки и вывод;
...
- запуск сгенерированной JVM-программы и создание исполняемого JAR (вместо этого обычный Funo выполняет встроенный интерпретатор);
- выполнение обработчиков Minecraft, Gradle-сборка JAR Minecraft-мода и Minecraft Launcher;
...
Эти элементы скрыты в мобильном интерфейсе, а frontend и Rust backend дополнительно отклоняют прямой вызов неподдерживаемой операции. Обычный проект можно запустить кнопкой **Run** прямо на телефоне. Minecraft-проект на Android можно редактировать, проверить и открыть как Java-предпросмотр, но готовый JAR следует собирать в desktop-версии или CI.

Интерпретатор не использует `eval` и не запускает произвольные команды. Для защиты приложения от зависания один запуск ограничен 250 000 операций, глубиной 128 вызовов, 256 КиБ вывода и 10 000 элементов коллекции. При превышении лимита Studio останавливает программу с понятной ошибкой. Интерактивные `readln`/`readInt` и Java-библиотеки в мобильном интерпретаторе пока не подключены.
