...
Funo Studio также собирается как телефонный Tauri 2 APK. На Android обычные Funo-программы действительно выполняются встроенным process-free Rust-интерпретатором без JDK: работают функции и рекурсия, переменные, условия, циклы, массивы, коллекции, f-строки и вывод. Также доступны редактор, локальное автосохранение полного проекта, файлы и папки, проверка Rust-компилятором, Java-предпросмотр, source-only мастер Minecraft, вики, обучение и сетевые каталоги. Интерфейс использует нижнюю навигацию, выдвижной проводник, touch-элементы и safe-area отступы.

JDK/Gradle, запуск сгенерированного JVM/JAR, выполнение и сборка Minecraft-модов, системные compiler backend-ы, установка `.jar`/модов, PATH, Microsoft-вход лаунчера и Plugin SDK требуют desktop-среды и в APK скрыты или явно отклоняются. Мобильный интерпретатор имеет лимиты операций, рекурсии, вывода и размера коллекций, чтобы ошибочная программа не повесила приложение.
