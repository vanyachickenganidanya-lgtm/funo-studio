...
const wiki = [
  ...
  ['Minecraft', 'События и команды', `<h1>Minecraft API Funo</h1><p>Мастер создаёт настоящий Fabric/Forge/NeoForge Gradle-проект, мост событий и Funo API.</p><pre>use minecraft.fabric\n\nmod "hello_mod" {\n    on start {\n        log("Мод загружен")\n    }\n    on server_start {\n        broadcast("Сервер готов!")\n        run_command("time set day")\n    }\n    on player_join(player) {\n        tell("Добро пожаловать!")\n        give("minecraft:diamond", 1)\n        damage(2)\n        tp("~", "~1", "~")\n    }\n}</pre><div class="doc-note"><code>tell</code>, <code>give</code>, <code>damage</code> и <code>tp</code> действуют только на игрока, для которого выполняется событие. Для <code>damage</code> нужен Minecraft 1.19.4 или новее. Одиночный мир — это встроенный локальный сервер: вы одновременно хост и подключившийся игрок, поэтому <code>player_join</code> срабатывает и там.</div>`],
