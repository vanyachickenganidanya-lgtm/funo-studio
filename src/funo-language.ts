...
export function registerFunoLanguage() {
  ...
  monaco.languages.setMonarchTokensProvider('funo', {
    defaultToken: '',
    tokenPostfix: '.funo',
    keywords: ['fun', 'let', 'var', 'const', 'if', 'then', 'else', 'return', 'use', 'java', 'lib', 'public', 'mod', 'on', 'start', 'server_start', 'player_join', 'player_leave', 'player_tick', 'block_break', 'block_place', 'block_interact', 'item_use', 'item_pickup', 'item_drop', 'item_craft', 'item_smelt', 'entity_interact', 'entity_attack', 'entity_kill', 'player_damage', 'player_death', 'player_respawn', 'dimension_change', 'chat', 'command', 'container_open', 'container_close', 'player_sleep', 'player_wake', 'advancement', 'player_jump', 'player_event', 'for', 'in', 'while', 'repeat', 'break', 'continue', 'new', 'true', 'false', 'null', 'and', 'or', 'not'],
    typeKeywords: ['byte', 'short', 'int', 'long', 'float', 'double', 'number', 'decimal', 'text', 'string', 'bool', 'boolean', 'char', 'list', 'set', 'map', 'any', 'void'],
    builtins: ['println', 'print', 'readln', 'readInt', 'readLong', 'readDouble', 'readBool', 'toInt', 'toDouble', 'len', 'list', 'set', 'map', 'minecraft', 'fabric', 'forge', 'neoforge', 'log', 'broadcast', 'tell', 'give', 'give_custom', 'damage', 'tp', 'define_item', 'craft_shapeless', 'craft_shaped', 'spawn_mob', 'set_mob_ai', 'mob_attribute', 'run_command', 'actionbar'],
    operators: ['=', '>', '<', '!', '~', '?', ':', '==', '<=', '>=', '!=', '&&', '||', '+', '-', '*', '/', '^', '%', '->'],
    symbols: /[=><!~?:&|+\-*\/\^%]+/,
    tokenizer: {
      root: [
  ...
  monaco.languages.registerCompletionItemProvider('funo', {
    ...
    provideCompletionItems(model, position) {
      ...
      return { suggestions: [
        ...
        item('f-string', 'Строка со значениями выражений на всех backend', 'f"${1:Значение}: {${2:value}}"', monaco.languages.CompletionItemKind.Snippet),
        ...
        item('on block_break', 'Игрок сломал/добыл блок', 'on block_break(player, block) {\n    ${1:tell(f"Сломан блок {block}")}\n}'),
        item('on player_leave', 'Игрок покинул мир', 'on player_leave(player) {\n    ${1:log(f"{player} вышел")}\n}'),
        item('on player_event', 'Универсальный fallback события игрока', 'on player_event(player, event, detail) {\n    ${1:log(f"{player}: {event} — {detail}")}\n}'),
        item('damage', 'Нанести урон только игроку текущего события', 'damage(${1:2})', monaco.languages.CompletionItemKind.Function),
        item('tp', 'Телепортировать только игрока текущего события', 'tp(${1:0}, ${2:80}, ${3:0})', monaco.languages.CompletionItemKind.Function),
  ...
  monaco.languages.registerHoverProvider('funo', {
    provideHover(model, position) {
      ...
      const docs: Record<string, string> = {
        ...
        println: '**println(value)** выводит значение и переходит на новую строку. Поддерживает f-строки: `println(f"Счёт: {score}")`.',
        f: '**f"...{expression}..."** — f-строка Funo. Работает в Java, Minecraft, C++, Rust, JavaScript и Python; `{{` и `}}` выводят фигурные скобки.',
        ...
        player_join: '**on player_join(player)** срабатывает при входе в мир. `player` — именно вошедший игрок.',
        player_leave: '**on player_leave(player)** срабатывает, когда игрок покидает мир.',
        block_break: '**on block_break(player, block)** срабатывает при добыче/разрушении блока; `player` — исполнитель, `block` — контекст блока.',
        player_event: '**on player_event(player, event, detail)** — универсальный version-neutral fallback всех доступных внутримировых событий игрока.',
        give: '**give(item, count)** выдаёт предмет игроку текущего события Minecraft.',
        damage: '**damage(amount, damageType?)** наносит урон только игроку текущего события в Minecraft 1.19.4+. `damage(2)` снимает одно сердце; необязательно можно указать тип урона.',
        tp: '**tp(x, y, z)** телепортирует только игрока текущего события. Для относительных координат используйте строки: `tp("~", "~5", "~")`.',
