...
fn rust_parameters(value: &str) -> (String, String) {
    ...
    for part in split_generic(value).into_iter().filter(|part| !part.trim().is_empty()) {
        ...
        } else if matches!(kind, "double" | "number" | "decimal") {
            parameters.push(format!("{name}: impl Into<f64>"));
            conversions.push_str(&format!("    let {name}: f64 = {name}.into();\n"));
        } else {
...
fn update_parameter_floats(parameters: &str, float_names: &mut HashSet<String>) {
    float_names.clear();
    for parameter in split_generic(parameters) {
        let Some((name, kind)) = parameter.split_once(':') else { continue };
        if matches!(kind.trim(), "float" | "double" | "number" | "decimal") {
            float_names.insert(name.trim().to_string());
        }
    }
}

fn coerce_rust_float_comparisons(value: &str, float_names: &HashSet<String>) -> String {
    let mut value = value.to_string();
    for name in float_names {
        let name = regex::escape(name);
        let right = Regex::new(&format!(
            r"(\b{name}\b\s*(?:<=|>=|==|!=|<|>)\s*)(\d+)(\s*(?:$|&&|\|\||\band\b|\bor\b|\)))"
        ))
        .unwrap();
        value = right.replace_all(&value, "${1}${2}.0${3}").into_owned();
        let left = Regex::new(&format!(
            r"(^|(?:&&|\|\||\band\b|\bor\b|\()\s*)(\d+)(\s*(?:<=|>=|==|!=|<|>)\s*\b{name}\b)"
        ))
        .unwrap();
        value = left.replace_all(&value, "${1}${2}.0${3}").into_owned();
    }
    value
}

fn rust_condition(value: &str, float_names: &HashSet<String>) -> String {
    conditional_expression(&coerce_rust_float_comparisons(value, float_names), "rust")
}

fn rust_expression_for_type(value: &str, kind: Option<&str>, float_names: &HashSet<String>) -> String {
    ...
    if let Some(parts) = conditional.captures(value.trim()) {
        let condition = rust_condition(&parts[1], float_names);
        let yes = rust_expression_for_type(&parts[2], kind, float_names);
        let no = rust_expression_for_type(&parts[3], kind, float_names);
...
fn transpile_rust(source: &str) -> Result<String, String> {
    ...
    for raw in source.lines() {
        ...
        let indent = "    ".repeat(raw.chars().take_while(|value| value.is_whitespace()).count() / 4);
        update_collection_kind(line, &collection_typed, &collection_named, &mut set_names);
        if let Some(cap) = typed.captures(line) {
            let name = cap[2].to_string();
            if matches!(&cap[1], "float" | "double" | "number" | "decimal") {
                float_names.insert(name);
            } else {
                float_names.remove(&name);
            }
        } else if let Some(cap) = named.captures(line) {
            if let Some(kind) = cap.get(3) {
                let name = cap[2].to_string();
                if matches!(kind.as_str().trim(), "float" | "double" | "number" | "decimal") {
                    float_names.insert(name);
                } else {
                    float_names.remove(&name);
                }
        ...
        if let Some(cap) = function.captures(line) {
            update_parameter_collections(&cap[2], &mut set_names);
            update_parameter_floats(&cap[2], &mut float_names);
            ...
            if let Some(value) = expression.strip_prefix("println(").and_then(|value| value.strip_suffix(')')) {
                ...
                out.push_str(&format!("    {}\n", rust_expression_for_type(expression, cap.get(3).map(|value| value.as_str()), &float_names)));
            }
            out.push_str("}\n");
        } else if let Some(cap) = typed.captures(line) {
            let value = rust_expression_for_type(&cap[3], Some(&cap[1]), &float_names);
            ...
            let value = rust_expression_for_type(&cap[4], declared_type, &float_names);
            ...
            out.push_str(&format!("{indent}return {};\n", rust_expression_for_type(value, current_return_type.as_deref(), &float_names)));
        } else if let Some(condition) = line.strip_prefix("if ").and_then(|v| v.strip_suffix('{')) {
            out.push_str(&format!("{indent}if {} {{\n", rust_condition(condition.trim(), &float_names)));
        } else if let Some(condition) = line.strip_prefix("while ").and_then(|v| v.strip_suffix('{')) {
            out.push_str(&format!("{indent}while {} {{\n", rust_condition(condition.trim(), &float_names)));
        } else if let Some(condition) = line.strip_prefix("} else if ").and_then(|v| v.strip_suffix('{')) {
            out.push_str(&format!("{indent}}} else if {} {{\n", rust_condition(condition.trim(), &float_names)));
