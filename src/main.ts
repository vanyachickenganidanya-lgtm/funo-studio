...
function renderMinecraft() {
  showSurface(`<div class="surface-page minecraft-page"><div class="page-hero"><div><span class="overline">MINECRAFT + FUNO</span><h1>Новый мод без сложного Java-кода</h1><p>Funo создаст Gradle-проект, описание мода и простую точку запуска.</p></div><div class="voxel">F</div></div>
    ...
    <section class="code-preview"><span>main.fun</span><pre><i>use</i> minecraft.<b id="loaderCode">fabric</b>\n\n<i>mod</i> <s>"my_first_mod"</s> {\n    <i>on</i> start {\n        <em>println</em>(<s>"Мод запущен!"</s>)\n    }\n}</pre><div class="what-created"><b>Funo создаст:</b><span>✓ build.gradle</span><span>✓ fabric.mod.json / mods.toml</span><span>✓ Java-мост загрузчика</span><span>✓ исходник Funo</span></div></section></div>
    <div class="learn-strip"><div>${icon('spark')}</div><p><b>Можно использовать Java API напрямую.</b><br>Напишите <code>use java "net.minecraft…"</code> или установите адаптер из Funo Pack.</p></div>
  </div>`);
  let loader = 'fabric';
...
const wiki = [
  ['Старт', 'Самый короткий код', `<h1>Самый короткий код</h1><p>Funo сам выводит типы и возвращаемые значения.</p><pre>fun hello(name) = "Привет, " + name\n\nfun main() = println(hello("Мир"))</pre><div class="doc-note">Точки с запятой и <code>return(200)</code> необязательны.</div>`],
  ['Основы', 'Функции', `<h1>Функции</h1><p>Ключевое слово <code>fun</code> создаёт функцию.</p><pre>fun double(n) = n * 2\n\nfun main() {\n    println(double(10))\n}</pre><h2>Явные типы для профи</h2><pre>fun double(n: int) -> int = n * 2</pre>`],
  ['Java', 'Java-библиотеки', `<h1>Java-библиотеки</h1><p>Установите Maven-пакет или .jar, затем импортируйте класс.</p><pre>use java "com.google.gson.Gson"\n\nfun main() {\n    gson = new Gson()\n    println(gson.toJson("Привет"))\n}</pre>`],
  ['Minecraft', 'Первый мод', `<h1>Первый Minecraft-мод</h1><p>Выберите значок куба слева. Мастер создаст Gradle-проект и точку входа.</p><pre>use minecraft.fabric\n\nmod "hello_mod" {\n    on start {\n        println("Мод запущен!")\n    }\n}</pre>`],
  ['Пакеты', 'Официальный реестр', `<h1>Официальный реестр</h1><p>Funo загружает список из вашего репозитория:</p><pre>github.com/vanyachickenganidanya-lgtm/funo_libsOFFICAL</pre><p>Для реестра нужен <code>index.json</code>. Готовый пример находится в папке <code>registry-template</code>.</p>`]
];
...
function renderSettings() {
  showSurface(`<div class="surface-page settings-page"><div class="page-hero"><div><span class="overline">НАСТРОЙКИ</span><h1>Funo Studio</h1><p>Редактор хранит исходники локально. Код не отправляется в облако.</p></div></div><div class="settings-list"><section><h2>Редактор</h2><label><span><b>Режим по умолчанию</b><small>В новичке объяснения подробнее</small></span><select><option>Новичок</option><option>Профи</option></select></label><label><span><b>Автосохранение</b><small>После короткой паузы</small></span><input type="checkbox" checked></label></section><section><h2>Компилятор</h2><label><span><b>Java</b><small>Путь определяется автоматически</small></span><input value="javac"></label><label><span><b>Успешный код Funo</b><small>return(200) преобразуется в exit 0</small></span><input value="200"></label></section><section><h2>Пакеты</h2><label><span><b>Официальный GitHub</b><small>Индекс проверенных библиотек</small></span><input value="vanyachickenganidanya-lgtm/funo_libsOFFICAL"></label></section></div></div>`);
}
